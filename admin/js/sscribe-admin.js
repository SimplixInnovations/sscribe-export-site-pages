...
(function ($) {
	...
	const SScribe = {
		...
		refreshStatusAndLanguageCounts: function (postType, language) {
			...
			$( 'input[name="sscribe_language"]' ).each(
				function () {
		...
		updateStatusCounts: function (counts) {
			...
			$( '.sscribe-status-card-label' ).each(
				function () {
					const $label = $( this );
		...
		updateConfigSummary: function () {
			...
			this._configSummaryDebounceTimer = setTimeout(
				function () {
		...
		showPreflightWarnings: function (diagnostics, onProceed) {
			...
			$banner.on(
				'click.sscribe-preflight',
				'.sscribe-preflight-proceed',
				function () {
					$banner.fadeOut(
						200,
						function () {
					...
					self._finalizingXHR = $.ajax(
						{
							...
							success: function (response) {
								...
								if (response.data && response.data.code === 'not_finalizing') {
									...
									self.findExportInRecentExports(
										resultData,
										function (matched) {
											if (matched) {
		...
		renderRecentExports: function (exports) {
			...
			$table.find( '.sscribe-delete-btn' ).each(
				function () {
					const $btn = $( this );
		...
		showToast: function (message, type, duration) {
			...
			$toast.find( '.sscribe-toast-dismiss' ).on(
				'click',
				function () {
		...
		renderSupportInfo: function (data) {
			...
			if (data.sections) {
				Object.keys( data.sections ).forEach(
					function (sectionKey) {
						const section = data.sections[sectionKey];
						if (section && section.items) {
							Object.keys( section.items ).forEach(
								function (itemKey) {
		...
		renderExportLog: function (data) {
			...
			const pages    = Object.keys( pagesObj ).map(
				function (key) {
					return pagesObj[key];
				}
			);
				...
				pages.forEach(
					function (page) {
						...
						if (page.formats && typeof page.formats === 'object') {
							const formatKeys = Object.keys( page.formats ).filter(
								function (fmt) {
	 ...
	 */
	$( document ).ajaxError(
		function (_event, jqXHR, _settings, exception) {
			let requestData = null;
			if (_settings && _settings.data) {
				if (typeof _settings.data === 'string') {
					requestData = {};
					_settings.data.replace(
						/([^&=]+)=([^&]*)/g,
						function (_, key, val) {
