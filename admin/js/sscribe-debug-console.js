			...
			this.$clearBtn.on(
				...
				function () {
					...
					if ($btn.data( 'confirming' )) {
						...
						self.clearBtnTimeout = setTimeout(
							function () {
								if (self.$clearBtn) {
									self.$clearBtn
				...
				function (response) {
					...
					if (response.success) {
						...
						response.data.debug_enabled !== sentDebugEnabled
							...
							.addClass( 'success' );
							self.saveFeedbackTimeout = setTimeout(
								function () {
			...
			this.downloadViaFetch(
				...
				data,
				{
					...
					onError: function () {
						...
						self.exportHintTimeout = setTimeout(
							function () {
								self.hidePausedIndicator();
								self.exportHintTimeout = null;
							},
...
(function ($) {
	...
	function ssDebugEnhanceLayout() {
		...
		$( '#sscribe-debug-filter-level, #sscribe-debug-session-id, #sscribe-debug-search' ).each(
			function () {
				$( this ).css(
					{
						'height': '36px',
		...
		$root.find( 'label[for="sscribe-debug-search"]' ).each(
			function () {
				var $lbl = $( this );
		...
		$root.find( '.sscribe-debug-search-label, [data-label="search"]' ).each(
			function () {
		...
		var $disclosureToggle = $root.find( '[aria-expanded]' ).filter(
			function () {
...
(function ($) {
	...
	function ssObserveContextPanels() {
		...
		var entriesObserver = new MutationObserver(
			function () {
				observeAll();
			}
		);
...
(function ($) {
			...
			function (row, i) {
				...
				tds.forEach(
					function (td, j) {
						...
						if (i === 0) {
							style = style.replace( 'border-top:1px solid rgb(255 255 255 / 7%);', '' );
						}
	...
	$( document ).on(
		'click',
		'#sscribe-debug-entries .sscribe-debug-entry, #sscribe-debug-entries [role="button"]',
		function () {
			var entry = this;
			setTimeout(
				function () {
