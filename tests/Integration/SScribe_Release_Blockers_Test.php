...
final class SScribe_Release_Blockers_Test extends TestCase {
	...
	public function test_release_blocker_checklist_lists_every_canonical_blocker(): void {
		...
		// Each blocker must appear somewhere in the document body
		// (case-insensitive substring match — the table can wrap
		// the label across lines, and the row count may grow in
		// future releases).
	...
	public function test_every_release_blocker_has_resolved_or_deferred_status(): void {
		...
		$hits = array();
		if ( preg_match_all( '/^\|\s*([0-9]+)\s*\|\s*([^|]+?)\s*\|\s*(RESOLVED|DEFERRED|OPEN|BLOCKED)\s*\|/m', $src, $matches ) ) {
