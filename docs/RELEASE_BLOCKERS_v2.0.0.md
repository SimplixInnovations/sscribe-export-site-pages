...
| 3  | PHPUnit runtime fatal                                                                     | RESOLVED  | 1457 tests, 6090 assertions, 0 failures, 0 deprecations, 28 skipped (deterministic across 3 consecutive random-order runs); fixes shipped: (a) `SScribe_WPML_Strategy_Test::test_page_collector_is_wpml_active_returns_false_when_wpml_absent` doc-comment metadata (`@runInSeparateProcess` / `@preserveGlobalState`) replaced with PHP 8 attributes; (b) `SScribe_Download_Security_Test::test_manifest_records_all_ten_rules` made order-independent via `setUpBeforeClass` that re-spawns the verifier subprocess to seed `dist/download-security-manifest.json`. |
