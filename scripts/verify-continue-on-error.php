...
if ( ! preg_match( '/SScribe_Activator::create_database_tables\s*\(\s*false\s*\)/', $upgrader_src ) ) {
	$errors[] = 'Upgrader must reuse SScribe_Activator::create_database_tables(false) so fresh installs and upgrades cannot drift onto different schemas.';
}
