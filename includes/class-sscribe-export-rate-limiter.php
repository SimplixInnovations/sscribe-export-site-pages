...
class SScribe_Export_Rate_Limiter {
	...
	public function check_rate_limit_decision( string $export_capability = 'sscribe_export', string $bucket = self::BUCKET_DEFAULT ): SScribe_Rate_Limit_Decision {
		...
		if ( (int) $data['count'] >= $rate_limit ) {
			...
			$this->release_lock( $option_lock_key, $lock_token );
	...
	/**
	 * Release the database-backed micro-lock if it is still owned by this request.
	 *
	 * Exact-value deletion prevents an expired owner from deleting a successor
	 * that acquired the same logical lock after lease turnover.
	 *
	 * @param string $option_key Lock option key.
	 * @param string $lock_token Owner token.
	 * @return void
	 */
	private function release_lock( string $option_key, string $lock_token ): void {
