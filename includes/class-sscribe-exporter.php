...
class SScribe_Exporter
{
    ...
    private function add_cover_page($section, $page_data)
    {
        ...
        $table->addRow();
        $cell = $table->addCell(Converter::inchToTwip(6.5), array('bgColor' => $this->colors['primary'], 'valign' => 'center'));
        $cell->addText(
            $this->safe_text(get_bloginfo('name') . ' | EXTERNAL AUDIT AND DOCUMENTATION'),
            array('name' => $this->font_name, 'size' => 10, 'color' => $this->colors['white'], 'bold' => true),
            array('alignment' => Jc::CENTER, 'spaceBefore' => Converter::pointToTwip(12), 'spaceAfter' => Converter::pointToTwip(12))
        );
        ...
        $info_table = $section->addTable(array(
            'borderSize' => 12,
        ...
        $meta_cell->addText(
            ...
            array('name' => $this->font_name, 'size' => 11, 'bold' => true, 'color' => $this->colors['heading']),
            array('alignment' => Jc::CENTER)
        );
        $meta_cell->addText(
            /* translators: %s: language code */
            sprintf(esc_html__('Target Language: %s', 'sscribe-export-site-pages'), $page_data['language']),
            array('name' => $this->font_name, 'size' => 10, 'color' => $this->colors['body']),
            array('alignment' => Jc::CENTER)
        );
        $meta_cell->addText(
        ...
        if (!empty($page_data['breadcrumbs'])) {
            $breadcrumb_text = implode(' > ', array_map(function ($c) {
                return $c['title'];
            }, $page_data['breadcrumbs']));
            $meta_cell->addTextBreak(1);
            $meta_cell->addText(
                'Site Path: ' . $this->safe_text($breadcrumb_text),
                array('name' => $this->font_name, 'size' => 9, 'color' => $this->colors['body'], 'italic' => true),
                array('alignment' => Jc::CENTER)
            );
        }
        ...
        $section->addText(
            'TABLE OF CONTENTS',
            array('name' => $this->font_name, 'size' => 16, 'bold' => true, 'color' => $this->colors['heading']),
            array('spaceAfter' => Converter::pointToTwip(12))
        );

        $section->addTOC(
            array('name' => $this->font_name, 'size' => 11),
    ...
    private function add_header_footer($section, $page_data)
    {
        ...
        $header_table->addCell(Converter::inchToTwip(3.25))->addText(
            ...
            array('name' => $this->font_name, 'size' => 8, 'color' => $this->colors['primary'], 'bold' => true)
        );
        $header_table->addCell(Converter::inchToTwip(3.25))->addText(
            $this->safe_text($page_data['title']),
            array('name' => $this->font_name, 'size' => 8, 'color' => $this->colors['body'], 'italic' => true),
        ...
        $footer_table->addCell(Converter::inchToTwip(4))->addText(
            $this->safe_text($page_data['permalink']),
            array('name' => $this->font_name, 'size' => 7, 'color' => $this->colors['body'])
        );
    ...
    private function render_element($section, $element)
    {
        ...
        switch ($element['type']) {
            ...
            case 'horizontal_rule':
                $section->addText(
                    str_repeat('—', 60),
                    array('size' => 8, 'color' => $this->colors['border']),
                    array('alignment' => Jc::CENTER)
                );
                break;
        }
    ...
    private function render_list($section, $element)
    {
        ...
        $list_type = ($style === 'numbered') ? \PhpOffice\PhpWord\Style\ListItem::TYPE_NUMBER : \PhpOffice\PhpWord\Style\ListItem::TYPE_BULLET_FILLED;
    ...
    private function render_button($section, $element)
    {
        ...
        $cell->addText(
            ...
            array('name' => $this->font_name, 'size' => 10, 'bold' => true, 'color' => $this->colors['primary']),
            array('alignment' => Jc::CENTER)
        );
        ...
        if (!empty($element['url'])) {
            $cell->addText(
                'DESTINATION URL:',
                array('name' => $this->font_name, 'size' => 8, 'bold' => true, 'color' => $this->colors['heading']),
                array('alignment' => Jc::CENTER, 'spaceBefore' => Converter::pointToTwip(6))
            );
            $cell->addLink(
                $element['url'],
                $this->safe_text($element['url']),
                array('name' => $this->font_name, 'size' => 9, 'color' => $this->colors['link'], 'underline' => 'single'),
                array('alignment' => Jc::CENTER)
            );
        }
    ...
    private function render_inline_image($section, $element)
    {
        ...
        if (empty($path) || !file_exists($path)) {
            ...
            $section->addText(
                ...
                array('name' => $this->font_name, 'size' => 9, 'italic' => true, 'color' => 'EF4444'),
                array('alignment' => Jc::CENTER)
            );
        } else {
            try {
        ...
        $cell->addText(
            ...
            array('name' => $this->font_name, 'size' => 7, 'bold' => true, 'color' => $this->colors['heading'])
        );
        $cell->addText(
            $this->safe_text($src),
            array('name' => 'Courier New', 'size' => 8, 'color' => $this->colors['link'])
        );
    ...
    private function add_child_pages($section, $page_data)
    {
        ...
        foreach ($page_data['children'] as $child) {
            $textRun = $section->addTextRun();
            $textRun->addText('> ', array('size' => 10, 'bold' => true, 'color' => $this->colors['primary']));
            $textRun->addLink(
                $child['url'],
